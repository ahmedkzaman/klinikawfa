...
- [ ] **Step 3: Implement the public token and type foundation**

Use the already CSP-allowlisted Google Fonts stylesheet with system fallbacks, then map semantic variables without changing their names:

```css
@import url("https://fonts.googleapis.com/css2?family=Atkinson+Hyperlegible:wght@400;700&family=Manrope:wght@500;600;700&display=swap");
...
Translate the six hex values to the existing semantic HSL variables so `primary`, `foreground`, `background`, `muted`, `accent`, and `whatsapp` retain their established API. Remove unused public glow/orb animation usage only after confirming no operational screen depends on the removed utility. Confirm `public/_headers` continues to allow only `https://fonts.googleapis.com` for styles and `https://fonts.gstatic.com` for fonts; do not broaden the CSP.

- [ ] **Step 4: Run design and existing foundation tests**
...
```bash
git add tailwind.config.ts src/index.css src/test/public-design-system.test.ts
...
- Mod�Mm�G����ƭy� the existing image source, opacity, CMS copy, carousel state, and preview guards; use a shorter responsive height, stronger foreground panel/contrast, two dominant actions, and restrained controls.
