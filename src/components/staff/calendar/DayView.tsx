...
export function DayView({ currentDate, tasks, leaveEntries, onTaskClick, onSlotClick }: DayViewProps) {
  ...
  return (
    <div className="border rounded-lg overflow-auto max-h-[calc(100vh-220px)]">
      {dayLeave.length > 0 && (
        <div className="grid grid-cols-[60px_1fr] border-b bg-rose-50">
      ...
      {HOURS.map((hour) => {
        ...
        return (
          <div key={hour} className="grid grid-cols-[60px_1fr] border-b min-h-[56px]">
            <div className="text-[10px] text-slate-500 text-right pr-2 pt-1 border-r">{format(new Date(2000, 0, 1, hour), 'h a')}</div>
            <div className="p-1 cursor-pointer hover:bg-slate-50 transition-colors space-y-0.5" onClick={() => { const d = new Date(currentDate); d.setHours(hour, 0, 0, 0); onSlotClick(d); }}>
