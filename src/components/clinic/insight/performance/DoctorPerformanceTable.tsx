...
type DoctorSortKey = 'completedVisits' | 'rosteredHours' | 'patientsPerHour' | 'visitBilling' | 'revenuePerHour' | 'procedures' | 'documents';
          ...
          <div className="grid gap-3 md:hidden">
            {sortedDoctors.map((doctor) => (
              <article key={doctor.doctorId ?? 'clinic-benchmark-mobile'} data-testid="doctor-performance-card" className="rounded-xl border border-slate-200 p-4">
                ...
                <dl className="mt-3 grid grid-cols-2 gap-2 text-sm"><div><dt className="text-xs text-slate-500">Rostered hours</dt><dd>{doctor.rosteredHours.toFixed(1)}</dd></div><div><dt className="text-xs text-slate-500">Patients / hour</dt><dd>{formatRate(doctor.patientsPerHour)}</dd></div>{showFinancialColumns ? <div><dt className="text-xs text-slate-500">Visit billing</dt><dd>{formatRM(doctor.visitBilling)}</dd></div> : null}<div><dt className="text-xs text-slate-500">Procedures · documents</dt><dd>{doctor.procedures} · {doctor.documents}</dd></div></dl>
