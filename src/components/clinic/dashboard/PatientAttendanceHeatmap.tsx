...
function cellStatus(cell: AttendanceHeatmapCell | undefined): 'closed' | 'insufficient' | 'covered' {
  if (!cell || cell.operatingOccurrences === 0) return 'closed';
...
function statusText(status: ReturnType<typeof cellStatus>): string {
  return status === 'closed' ? 'Closed / not operating' : status === 'insufficient' ? 'Insufficient data' : 'Covered average';
}
    ...
    <div className="space-y-4">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          {!invalidRange && !query.isLoading && !query.isError && cells.length === 0 && <p className="text-sm text-slate-500">No attendance data is available for this range.</p>}
          {!invalidRange && !query.isLoading && !query.isError && cells.length > 0 && <>
            <div className="flex flex-wrap gap-2 text-xs" aria-label="Heatmap legend"><span className="rounded bg-slate-100 px-2 py-1">Closed / not operating</span><span className="rounded bg-slate-200 px-2 py-1">Insufficient data</span><span className="rounded bg-blue-100 px-2 py-1 text-blue-950">Covered average (light to dark = lower to higher)</span><span className="rounded border-2 border-red-600 px-2 py-1 text-red-800">Wait alert (&gt;45 min)</span></div>
      ...
      {!query.isLoading && !query.isError && !invalidRange && <AttendanceRecommendations cells={cells} selectedDoctorId={doctorId} />}
