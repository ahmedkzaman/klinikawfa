...
describe('buildMalayAnnouncement', () => {
  ...
  it('lowercases mixed-case names in hidden speech text to avoid acronym spelling', () => {
    expect(buildMalayAnnouncement({
      callBy: 'name',
      display: 'Muhammad AQIL bin Zain',
      roomLabel: 'Bilik 1',
    })).toBe('Panggilan untuk muhammad aqil bin zain, sila ke Bilik 1 sekarang.');
  });
