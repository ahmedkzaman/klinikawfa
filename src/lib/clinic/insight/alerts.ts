...
export function buildClinicAlerts(metrics: ClinicHealthMetrics): ClinicHealthAlert[] {
  ...
  if (metrics.dataQuality.completedWithoutPayment > 0) alerts.push({ id: 'missing-payment', severity: 'warning', title: 'Completed visits without payment', detail: `${metrics.dataQuality.completedWithoutPayment} completed visits have no payment record.`, href: '/clinic/queue' });
