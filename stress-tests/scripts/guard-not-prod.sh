#!/usr/bin/env bash
# Refuses to run unless STAGING_PROJECT_REF is set and is NOT the production ref.
set -euo pipefail
...
if [[ "$STAGING_PROJECT_REF" == "$PROD_REF" ]]; then
  echo "FATAL: STAGING_PROJECT_REF == production ref ($PROD_REF). Refusing to run." >&2
  exit 2
fi
...
echo "OK: staging guard passed (ref=$STAGING_PROJECT_REF)"
