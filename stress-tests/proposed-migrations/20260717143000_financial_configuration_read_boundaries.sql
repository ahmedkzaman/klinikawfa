-- REVIEW ONLY: do not place under supabase/migrations until staging review
-- and explicit production approval are complete.
--
-- Restricts three configuration tables that currently expose sensitive bank,
-- payment-provider, tax, and negotiated panel terms to every authenticated
-- account. No rows or user-facing content are changed.
