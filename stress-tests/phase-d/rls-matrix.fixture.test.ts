/**
 * Deterministic per-role RLS matrix test.
 *
 * Fails at import time unless the guarded runner exported RLS_MATRIX_RUNNER=1.
 * Every SELECT test scopes to the reserved fixture IDs and asserts exact
 * sorted equality (no arrayContaining, no bare regex).
 */
...
if (process.env.RLS_MATRIX_RUNNER !== "1") {
  throw new Error(
    "rls-matrix.fixture.test.ts must only be executed through scripts/run-rls-matrix.sh"
  );
}
...
// Reserved fixture IDs (must match seed-rls-matrix.sql).
const FIX = {
  appt: {
...
const CRED: Record<Exclude<Role, "guest_anon">, { uid: string; email: string; pw: string }> = {
...
async function signIn(role: Exclude<Role, "guest_anon">): Promise<SupabaseClient> {
...
async function selectIds(
  sb: SupabaseClient, table: string, scope: string[]
): Promise<string[]> {
  const { data, error } = await sb.from(table).select("id").in("id", scope);
  expect(error).toBeNull();
  return sorted((data ?? []).map((r: any) => r.id as string));
}
...
test("[resident]      consultation_items → all active", async () => {
  expect(await selectIds(clients.resident!, "consultation_items", ITEM_IDS))
    .toEqual(sorted([FIX.item.locumActive, FIX.item.residentActive]));
});
...
test("[staff]         payments → all active", async () => {
  expect(await selectIds(clients.staff!, "payments", PAY_IDS))
    .toEqual(sorted([FIX.pay.locumActive, FIX.pay.residentActive]));
});
