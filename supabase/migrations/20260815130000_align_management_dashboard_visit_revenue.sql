-- Attribute monthly visit billing to the patient's actual queue-entry visit date.
-- This prevents historical/imported visits completed later from inflating the current month.
...
BEGIN
  ...
  WITH facts AS MATERIALIZED (
    SELECT fact.*
    FROM private.financial_control_visit_facts(v_month_start, v_as_of, v_as_of) AS fact
    JOIN public.queue_entries AS qe
      ON qe.id = fact.queue_entry_id
    WHERE (timezone('Asia/Kuala_Lumpur', qe.created_at))::date
