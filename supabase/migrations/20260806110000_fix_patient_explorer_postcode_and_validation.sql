...
BEGIN
  ...
  IF to_regprocedure('public.search_patient_explorer(jsonb, integer, integer)') IS NULL THEN
    RAISE EXCEPTION 'patient explorer repair preflight failed: expected RPC signature is missing';
  END IF;
