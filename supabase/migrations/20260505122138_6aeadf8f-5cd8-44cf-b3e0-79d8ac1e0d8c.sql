...
AS $function$
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
      AND role IN ('admin', 'staff', 'special_admin', 'doctor_admin', 'operations', 'locum', 'resident_doctor')
  )
$function$;
...
AS $function$
  SELECT EXISTS (
    ...
    WHERE user_id = _user_id
      AND role IN ('operations','admin','special_admin','doctor_admin','resident_doctor')
  )
$function$;
...
AS $function$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles
    WHERE user_id = _user_id
      AND role IN ('admin','special_admin','doctor_admin','operations','staff','resident_doctor')
  )
$function$;
