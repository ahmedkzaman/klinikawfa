...
BEGIN
  ...
  FROM (
    SELECT
      ...
      c.doctor_id,
      CASE
        WHEN c.doctor_id IS NULL THEN 'Unassigned'
        ELSE COALESCE(
          NULLIF(btrim(profile.full_name), ''),
          NULLIF(btrim(doctor.name), ''),
          'Unknown doctor'
        )
      END AS doctor_name,
      COALESCE(NULLIF(btrim(patient.name), ''), 'Unknown patient') AS patient_name
    ...
    LEFT JOIN public.profiles AS profile
      ON profile.id = c.doctor_id
    LEFT JOIN public.doctors AS doctor
      ON doctor.user_id = c.doctor_id
