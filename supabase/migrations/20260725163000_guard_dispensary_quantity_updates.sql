...
BEGIN
  ...
  IF jsonb_typeof(p_updates) <> 'object'
     OR p_updates - ARRAY[
       ...
       'dispensed_qty', 'partial_reason'
     ] <> '{}'::jsonb THEN
    RAISE EXCEPTION 'Unsupported dispensary item fields'
      USING ERRCODE = '22023';
  END IF;

  IF p_updates ? 'partial_reason'
     AND p_updates->>'partial_reason' IS NOT NULL
     AND p_updates->>'partial_reason' NOT IN ('patient_request', 'out_of_stock') THEN
    RAISE EXCEPTION 'Unsupported partial dispensing reason'
      USING ERRCODE = '22023';
  END IF;
  ...
  SET
    ...
    END,
    dispensed_qty = CASE
      WHEN p_updates ? 'dispensed_qty'
        THEN NULLIF(p_updates->>'dispensed_qty', '')::numeric
      ELSE dispensed_qty
    END,
    partial_reason = CASE
      WHEN p_updates ? 'partial_reason' THEN p_updates->>'partial_reason'
      ELSE partial_reason
    END
  WHERE id = p_item_id
