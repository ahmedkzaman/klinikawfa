...
BEGIN
  ...
  SELECT activity.* FROM (
    ...
    WHERE (ci.service_id IS NOT NULL OR ci.package_id IS NOT NULL OR lower(trim(COALESCE(ii.category, ''))) = 'procedure')
      AND ci.deleted_at IS NULL AND c.deleted_at IS NULL AND c.status = 'completed'
