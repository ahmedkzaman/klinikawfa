-- Patient Explorer: switch clinical filters from exact-match (= ANY) to
-- partial, case-insensitive matching (ILIKE '%term%').
--
-- Before this change, searching "amlodipine" required the stored item_name
-- to be EXACTLY "amlodipine" — but the database stores "Amlodipine 5mg Tablet",
-- so nothing matched. Same for diagnoses, procedures, blood investigations,
-- and attending doctor names.
--
-- The consultation_statuses filter keeps exact matching (statuses are enums:
-- "completed", "pending", etc.), and the blood-glucose vital-sign shortcut
-- keeps exact matching ("blood glucose").
-- All other clinical filters now use ILIKE partial matching so users can
-- type partial names like "amlodipine", "diabetes", or "ahmed".
          ...
          OR EXISTS (
            ...
            WHERE visit.patient_id = p.id
              AND EXISTS (
                ...
                WHERE lower(COALESCE(NULLIF(btrim(visit.diagnosis_text), ''), diagnosis.name, '')) ILIKE '%' || term || '%'
              )
          )
        )
        AND (
          ...
          OR EXISTS (
            ...
            WHERE visit.patient_id = p.id
              ...
              AND EXISTS (
                ...
                WHERE lower(COALESCE(NULLIF(btrim(item.item_name), ''), s.name, '')) ILIKE '%' || term || '%'
              )
            UNION ALL
            SELECT 1
            FROM matching_visits visit
          ...
          OR EXISTS (
            ...
            WHERE visit.patient_id = p.id
              ...
              AND EXISTS (
                ...
                WHERE lower(btrim(item.item_name)) ILIKE '%' || term || '%'
              )
          )
        )
        AND (
          ...
          OR EXISTS (
            ...
            WHERE visit.patient_id = p.id
              AND EXISTS (
                ...
                WHERE lower(COALESCE(NULLIF(btrim(doctor.name), ''), profile.full_name, '')) ILIKE '%' || term || '%'
              )
          )
        )
      GROUP BY p.id
...
COMMENT ON FUNCTION public.search_patient_explorer(jsonb, integer, integer) IS
  'Patient Explorer: partial ILIKE matching for diagnoses, blood investigations, procedures, medicines, and attending doctors. Consultation statuses remain exact-match. SECURITY INVOKER preserves RLS.';
