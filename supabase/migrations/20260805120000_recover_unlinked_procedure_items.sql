...
BEGIN
  ...
  SELECT activity.* FROM (
    SELECT ci.id, 'procedure'::text, (qe.created_at AT TIME ZONE 'Asia/Kuala_Lumpur')::date, COALESCE(s.name, ii.name, p.name, ci.item_name),
    ...
    FROM public.consultation_items ci JOIN public.consultations c ON c.id = ci.consultation_id
      JOIN public.queue_entries qe ON qe.id = c.queue_entry_id LEFT JOIN public.services s ON s.id = ci.service_id
      LEFT JOIN public.inventory_items ii ON ii.id = ci.item_id LEFT JOIN public.packages p ON p.id = ci.package_id
    ...
    WHERE (
        ci.service_id IS NOT NULL
        OR ci.package_id IS NOT NULL
        OR lower(trim(COALESCE(ii.category, ''))) = 'procedure'
        OR EXISTS (
          SELECT 1
          FROM public.services legacy_service
          WHERE lower(trim(legacy_service.category)) = 'procedure'
            AND lower(trim(legacy_service.name)) = lower(trim(ci.item_name))
        )
        OR lower(trim(ci.item_name)) IN ('excision biopsy', 'excision biopsy (procedure)')
      )
