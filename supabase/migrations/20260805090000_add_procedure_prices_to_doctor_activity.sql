...
BEGIN
  ...
  SELECT activity.* FROM (
    SELECT ci.id, 'procedure'::text, (qe.created_at AT TIME ZONE 'Asia/Kuala_Lumpur')::date,
      s.name, c.id, qe.id, qe.created_at, qe.queue_sequence, c.doctor_id,
      ...
      ci.price::numeric, COALESCE(ci.quantity, 1)::numeric, (ci.price * COALESCE(ci.quantity, 1))::numeric
    FROM public.consultation_items ci
    JOIN public.consultations c ON c.id = ci.consultation_id JOIN public.queue_entries qe ON qe.id = c.queue_entry_id
    JOIN public.services s ON s.id = ci.service_id LEFT JOIN public.doctors doctor ON doctor.id = c.doctor_id
    ...
    WHERE s.category = 'Procedure' AND ci.deleted_at IS NULL AND c.deleted_at IS NULL AND c.status = 'completed'
    ...
    FROM public.consultation_documents cd JOIN public.consultations c ON c.id = cd.consultation_id JOIN public.queue_entries qe ON qe.id = c.queue_entry_id
    LEFT JOIN public.doctors doctor ON doctor.id = c.doctor_id LEFT JOIN public.profiles profile ON profile.id = doctor.user_id LEFT JOIN public.patients patient ON patient.id = c.patient_id
  ...
  ) activity ORDER BY activity.activity_date, activity.activity_kind, activity.activity_id;
END;
$$;
