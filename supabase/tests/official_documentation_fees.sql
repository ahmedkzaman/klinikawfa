...
BEGIN
  ...
  BEGIN
    PERFORM public.issue_consultation_document_with_fee(
      ...
      'TEST ONLY CHANGED TEMPLATE',
      'prescription',
      'TEST ONLY CHANGED CONTENT',
      'A5',
      'landscape'
    );
    RAISE EXCEPTION 'CHANGED_IDEMPOTENCY_PAYLOAD_SUCCEEDED';
  EXCEPTION WHEN SQLSTATE '23505' THEN
    IF SQLERRM <> 'DOCUMENT_ID_CONFLICT' THEN
      RAISE;
    END IF;
  END;
  ...
  BEGIN
    ...
    SET consultation_id = '71000000-0000-4000-8000-000000000302',
        patient_id = '71000000-0000-4000-8000-000000000102',
        type = 'referral',
        created_by = '71000000-0000-4000-8000-000000000002'
    WHERE id = '71000000-0000-4000-8000-000000000401';
    RAISE EXCEPTION 'BILLED_DOCUMENT_REASSIGNMENT_SUCCEEDED';
  EXCEPTION WHEN SQLSTATE '42501' THEN
    IF SQLERRM <> 'BILLED_DOCUMENT_IMMUTABLE' THEN
      RAISE;
    END IF;
  END;
  ...
  BEGIN
    ...
    WHERE source_document_id = '71000000-0000-4000-8000-000000000401'
      AND deleted_at IS NULL;
    RAISE EXCEPTION 'DIRECT_LINKED_FEE_MUTATION_SUCCEEDED';
  EXCEPTION WHEN SQLSTATE '42501' THEN
    IF SQLERRM <> 'DOCUMENT_FEE_ITEM_IMMUTABLE' THEN
      RAISE;
    END IF;
  END;
  ...
  IF EXISTS (
    ...
    WHERE id = '71000000-0000-4000-8000-000000000401'
      AND (
        consultation_id IS DISTINCT FROM
          '71000000-0000-4000-8000-000000000301'::uuid
        OR patient_id IS DISTINCT FROM
          '71000000-0000-4000-8000-000000000101'::uuid
        OR type IS DISTINCT FROM 'mc'
        OR created_by IS DISTINCT FROM
          '71000000-0000-4000-8000-000000000001'::uuid
      )
  ) THEN
    RAISE EXCEPTION 'BILLED_DOCUMENT_CHANGED_AFTER_REJECTION';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE source_document_id = '71000000-0000-4000-8000-000000000401'
      ...
      AND (
        consultation_id IS DISTINCT FROM
          '71000000-0000-4000-8000-000000000301'::uuid
        OR price IS DISTINCT FROM 15
      )
  ) THEN
    RAISE EXCEPTION 'LINKED_FEE_CHANGED_AFTER_REJECTION';
  END IF;
